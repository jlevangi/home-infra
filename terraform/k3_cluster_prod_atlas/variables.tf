...
variable "vm_storage" {
  description = "Proxmox storage pool for OS disk + cloud-init drive. local-lvm = SSD-backed thin LVM on Atlas."
  type        = string
  default     = "local-lvm"
}
...
variable "data_disk_size" {
  description = "Longhorn data disk size. Sized from current /var/lib/longhorn usage (~170G) plus headroom."
  type        = string
  default     = "300G"
}
...
variable "ip_base" {
  description = "Base IP fragment; worker IPs are <ip_base>(index+1) -> .101/.102/.103"
  type        = string
}
