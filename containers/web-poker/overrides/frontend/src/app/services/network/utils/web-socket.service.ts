...
export class WebSocketService {
  ...
  protected setBasicConfig(server: string, port: number, endpoint: string) {
    ...
    this.stomp.configure({
      host: this.getScheme() + server + ':' + port + endpoint,
  ...
  protected startConnection() {
    this.wsEventSubscriptor.emit(new EventWS(EventTypeWS.CONNECTING, {host: this.getScheme() + this.server + ':' + this.port + this.endpoint}));
    this.stomp.startConnect().then(() => {
      this.stomp.done('init');
      this.isConnected = true;
      this.wsEventSubscriptor.emit(new EventWS(EventTypeWS.CONNECTED, {host: this.getScheme() + this.server + ':' + this.port + this.endpoint}));
    });
  }
