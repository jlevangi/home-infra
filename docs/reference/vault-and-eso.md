...
4. Change `argocd/apps/prod/external-secrets.yaml` to `targetRevision: 2.3.0`, set `installCRDs: false`, add `namespaceOverride: external-secrets` to the Helm values, and add `ServerSideApply=true` under `syncOptions`.
