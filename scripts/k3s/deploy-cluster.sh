...
if "$PROJECT_ROOT/scripts/k3s/helpers/k3s-context-manager.sh" setup > /dev/null 2>&1; then
...
if [ $RESULT -eq 0 ]; then
  ...
  if ! "$PROJECT_ROOT/scripts/k3s/helpers/k3s-context-manager.sh" setup; then
    echo "⚠️  Warning: Kubeconfig setup failed, but cluster deployment succeeded"
    echo "   You may need to run './scripts/k3s/helpers/k3s-context-manager.sh setup' manually"
  else
