...
#
# Usage:
#   ./scripts/k3s/deploy-component.sh --prod traefik          # Deploy Traefik to production
#   ./scripts/k3s/deploy-component.sh --test metallb          # Deploy MetalLB to test
#   ./scripts/k3s/deploy-component.sh --stage all-infra       # Deploy all infra to stage
#   ./scripts/k3s/deploy-component.sh --prod traefik --force  # Force redeploy
#   ./scripts/k3s/deploy-component.sh --list                  # List available components
...
if "$PROJECT_ROOT/scripts/k3s/helpers/k3s-context-manager.sh" switch "$KUBECTL_CONTEXT" 2>/dev/null; then
    ...
    echo "   Run '$PROJECT_ROOT/scripts/k3s/helpers/k3s-context-manager.sh setup' if cluster exists"
