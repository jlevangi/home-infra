...
#
# Usage:
#   ./scripts/maintenance/restore-cluster.sh --prod                 # Full DR for production
#   ./scripts/maintenance/restore-cluster.sh --stage                # Rebuild stage with prod data
#   ./scripts/maintenance/restore-cluster.sh --prod --restore-only  # Just restore data (no rebuild)
#   ./scripts/maintenance/restore-cluster.sh --stage --from prod    # Clone prod to stage
#
# =============================================================================
...
rebuild_vms() {
    ...
    if [[ -f "$REPO_ROOT/scripts/k3s/rebuild-cluster.sh" ]]; then
        "$REPO_ROOT/scripts/k3s/rebuild-cluster.sh" --$TARGET_ENV --yes
    else
...
deploy_k3s() {
    ...
    "$REPO_ROOT/scripts/k3s/deploy-cluster.sh" --$TARGET_ENV
}

switch_context() {
    print_step "Switching to $TARGET_ENV cluster context..."
    "$REPO_ROOT/scripts/k3s/helpers/k3s-context-manager.sh" switch "$TARGET_ENV"
}
