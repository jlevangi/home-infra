...
# Function to get pods with optional filtering
get_pods() {
    local search_term="$1"
    local namespace="$2"
...
extract_pod_info() {
    local selection="$1"
    # Extract pod name (first word) and namespace (after "ns:")
...
main() {
    ...
    if [[ ${#pods_array[@]} -eq 1 ]]; then
        ...
        selected_pod=$(printf '%s\n' "${pods_array[@]}" | fzf --prompt="Select pod to describe: " --height=20 --border --header="Use arrow keys to navigate, Enter to select")
